...
2. Read `${CLAUDE_PLUGIN_ROOT}/skills/design-architect/references/modes.md` for what redesign commits to, then `references/structures.md` and `references/direction-brief.md` to write the new brief — it must differ from the old stamp on structure and on at least one of palette or type — and `references/refuse-list.md` before emitting.
