...
2. Fetch the target **as a non-JS client** first, per `${CLAUDE_PLUGIN_ROOT}/skills/search-optimization-architect/references/technical-readiness.md` §1, and read the head for share tags in the same pass (§6). Run the Tier-1 checks and **stop the report there if one fails** — everything below is unreachable until it passes, but the share-preview result still ships: it came from the same fetch.
