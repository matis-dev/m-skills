...
3. Load `module-threat-model` → `references/triage.md` to triage each advisory by **reachability**, not by severity label alone. Load `module-gate-battery` between batches, `module-handover` for the revert and lockfile-rollback commands, and `guidelines-meta`.
